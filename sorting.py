...
def bubble_sort(lst):
    ...
    for big_i in range(len(lst) - 1):
        swapped = False
        for i in range(len(lst) - 1 - big_i):
            if lst[i] > lst[i+1]:
...
def merge_lists(list1, list2):
    ...
    while list1 or list2:
        if list1 == []:
            third_list.append(list2.pop(0))
        elif list2 == []:
            third_list.append(list1.pop(0))
...
def merge_sort(lst):
    ...
    list1 = merge_sort(lst[:midway])
    list2 = merge_sort(lst[midway:])
